...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=14, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # TODO whether use Conv2d or Linear
        # else:
        #     self.patch_embed = PatchEmbed_2d(
        #         col_patch_num=col_patch_num, row_patch_num=row_patch_num, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
        else:
    ...
    # def interpolate_2d_pos_encoding(self, x, w, h):
    #     npatch = x.shape[1] - 1
    #     # N = npatch 
    #     N = self.pos_embed.shape[1] - 1
    #     if npatch == N and w == h:
    ...
    def forward_features(self, x: torch.Tensor, position_ids, patch_shape, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        if self.use_2d_pos_emb:
            ...
            col_embedding = self.col_pos_emb(position_ids[:, :, 0])
            row_embedding = self.row_pos_emb(position_ids[:, :, 1])
